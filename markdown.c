...
off_t md_listdir(char *d, int *dt, int *ft, u_long lev, dev_t dev)
{
  char *path;
  ...
  while(*dir) {
    ...
    if (nolinks) fprintf(outfile,"%s    ",path);
    else{
      fprintf(outfile,"[%s](./%s)    ",path,path);
    }
...
off_t md_rlistdir(char *d, int *dt, int *ft, u_long lev, dev_t dev)
{
  struct _info **dir;
  off_t size = 0;
  char *err;
  ...
  mdr_listdir(dir, d, dt, ft, lev);

  return size;
}

void mdr_listdir(struct _info **dir, char *d, int *dt, int *ft, u_long lev)
{
  char *path;
  long pathsize = 0;
  ...
  while(*dir) {
	...
	if (!strcmp(d,"/")) sprintf(path,"%s%s",d,(*dir)->name);
      ...
      mdr_listdir((*dir)->child, fflag? path : NULL, dt, ft, lev+1);
      nlf = TRUE;
      *dt += 1;
    } else {
      if ((*dir)->isdir) *dt += 1;
