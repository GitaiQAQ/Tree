...
bool Hflag, siflag, cflag, Xflag, Jflag, MDflag,duflag, pruneflag;
bool noindent, force_color, nocolor, xdev, noreport, nolinks, noframework,flimit, dirsfirst;
...
int main(int argc, char **argv)
{
  ...
  noindent = force_color = nocolor = xdev = noreport = nolinks = noframework = reverse = FALSE;
  ignorecase = matchdirs = dirsfirst = inodeflag = devflag = Xflag = Jflag = MDflag= FALSE;
...
/* Until I get rid of this hack, make it linux/cygwin/HP nonstop only: */
  ...
  if (Hflag) {
    ...
    listdir = needfulltree ? md_rlistdir :md_listdir;
    Xflag = FALSE;
  } else if (Xflag) {
  ...
  if (Hflag) {
    ...
    if (!noframework)
    {
		...
		fprintf(outfile,hversion,linedraw->copy, linedraw->copy, linedraw->copy, linedraw->copy, linedraw->copy);
		fprintf(outfile,"\t</p>\n");
		...
		fprintf(outfile,"<hr>\n");
		fprintf(outfile,"<p class=\"VERSION\">\n");
		fprintf(outfile,hversion_noframework,linedraw->copy, linedraw->copy, linedraw->copy, linedraw->copy, linedraw->copy);
		fprintf(outfile,"</p>\n");
    }
  } else if (MDflag) {
    fprintf(outfile,hversion_md,linedraw->copy, linedraw->copy, linedraw->copy, linedraw->copy, linedraw->copy);
  } else if (Xflag) {
  ...
  if (dirname) {
    ...
    else if (MDflag) fprintf(outfile,".    ");
...
void usage(int n)
{
  ...
  fprintf(n < 2? stderr: stdout,
	...
	"\t[--help] [--inodes] [--device] [--noreport] [--nolinks] [--noframework][--dirsfirst]\n"
  ...
  fprintf(stdout,
	...
	"  ------- XML/HTML/JSON/Markdown options -------\n"
	"  -X            Prints out an XML representation of the tree.\n"
	"  -J            Prints out an JSON representation of the tree.\n"
	"  -M            Prints out an Markdown representation of the tree.\n"
	"  -H baseHREF   Prints out HTML format with baseHREF as top directory.\n"
	"  -T string     Replace the default title and H2 header with string in HTML or Markdown output.\n"
	"  --nolinks     Turn off hyperlinks in HTML or Markdown output.\n"
	"  --noframework Only content in HTML output.\n"
...
void indent(int maxlevel)
{
  ...
  if (ansilines) {
    ...
    if (Hflag&&!noframework) fprintf(outfile,"\t");
    for(i=0; dirs[i] && i <= maxlevel; i++) {
      fprintf(outfile,"%s ",
